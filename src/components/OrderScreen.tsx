...
const OrderScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const renderOrderItem = ({ item }: { item: OrderItem }) => (
    <TouchableOpacity
      style={[styles.orderContainer, selectedOrder?.id === item.id && styles.selectedOrder]}
...
const styles = StyleSheet.create({
  ...
  topBar: {
    flexDirection: 'row',
    justifyContent: 'space-between', // Change to space-between to place the Orders Text on the right
    alignItems: 'center',
    height: 120,
    backgroundColor: '#242428',
